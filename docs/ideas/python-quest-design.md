...
#### 無料枠で運用できる範囲（生徒 〜50人規模）

| サービス | プラン | 月額 | 備考 |
|---------|--------|------|------|
| **Vercel** | Hobby（無料） | **¥0** | 個人プロジェクト。帯域100GB/月、サーバーレス実行10万回/月 |
...
#### スケールアップ時（生徒 50〜500人規模）

| サービス | プラン | 月額 | 備考 |
|---------|--------|------|------|
| **Vercel** | Pro | **$20（約 ¥3,000）** | 帯域1TB、チーム利用、プレビューデプロイ |
...
| **イラスト・キャラ素材** | ¥0〜¥30,000 | 絵文字ベースなら無料。オリジナルイラスト発注時 |
| **フォント** | ¥0 | Google Fonts で無料 |
| **SSL証明書** | ¥0 | Vercel / GitHub Pages が自動対応 |

### 10.4 コストまとめ

| フェーズ | 想定規模 | 月額コスト |
|---------|---------|-----------|
| **MVP〜Phase 2** | 〜50人 | **¥0**（完全無料で運用可能） |
| **Phase 3（本格運用）** | 50〜500人 | **約 ¥6,750/月** |
| **大規模展開** | 500人〜 | **約 ¥7,000〜/月** |

> **📌 結論:** MVP段階は完全無料で開発・運用可能。生徒50人程度までは無料枠で十分カバーできる。Python実行をブラウザ側で行う設計のおかげで、サーバーコストを最小限に抑えられるのが本プラットフォームの強み。
